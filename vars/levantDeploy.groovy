...
def getJobsRepo() {
    return 'https://github.com/Void9711/sgsdk-live.git'
}
...
def call(Map m = [:]) {
    ...
    def nomadJobVars = useNomadJobVars ? "-var-file=./vars/${nomadVars}/${jobName}.yml" : ''
            ...
            withCredentials([string(variable: 'NOMAD_TOKEN', credentialsId: nomadToken)]) {
                withEnv(["NOMAD_ADDR=${nomadAddr}"]) {
                    sh """
                        ...
                        -var-file=./vars/${nomadVars}/common.yml \
                        ${nomadJobVars} \
                        ./jobs/${jobName}.nomad ${nomadRun}
                    """
                }
            }
        }
